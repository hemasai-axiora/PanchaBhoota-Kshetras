...
const seed = async () => {
  try {
    ...
    const adminEmail = 'admin@panchabhoota.org';
    ...
    console.log(`Admin seeded! Login: ${adminEmail} / Shiva@108`);

    // 2. Seed Temples
    console.log('Seeding temples data...');
    await Temple.deleteMany({});
    const seededTemples = await Temple.insertMany(templesData);
    console.log(`${seededTemples.length} temples successfully seeded into database!`);

    // 3. Clear Contacts
    console.log('Cleaning contact messages...');
    ...
    await Contact.create({
      ...
      subject: 'Spiritual Seekers Group Visit',
      message: 'Namaste! We are planning a group spiritual tour to all 5 Pancha Bhoota Kshetras this coming October with 15 devotees. Can you suggest an ideal itinerary starting from Kanchipuram and ending at Chidambaram? Thank you!',
      status: 'Unread'
    });
